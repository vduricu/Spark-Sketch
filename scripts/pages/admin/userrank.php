...
$clean['rank'] = filter_input(INPUT_POST,"rank",FILTER_SANITIZE_STRING);

if(empty($clean['user'])||empty($clean['rank']))
...
mysql_query("UPDATE `user` SET `rank`='{$clean['rank']}' WHERE `user`='{$clean['user']}' LIMIT 1");
masterDie("good");
