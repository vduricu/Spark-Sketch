...
<div class="right" id="content">
	<h1 class="title"><?php ___("mygalleryTitle")?></h1>
	<div class="content">
		<table>
		<?php
		$userid = loggedUserID();
		$q = mysql_query("SELECT * FROM `draws` WHERE `userid`='{$userid}' ORDER by `modificationdate` DESC");
		if(!mysql_num_rows($q))
			...
			while($r = mysql_fetch_assoc($q)){
				...
				echo "<td style=\"width: 23%; padding:1%;\"><a href=\"{$url}\"><img src=\"{$image}\" title=\"{$r['title']}\" alt=\"image\" style=\"width: 100%;border:1px solid #000000\"/></a></td>\n";
