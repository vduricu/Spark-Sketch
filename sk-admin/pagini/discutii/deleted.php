<div class="content adminContent">
	<div class="aSMenu">
		...
		|&nbsp;<a href="./?zona=discutii&pagina=spam">Mesaje spam <span>(<?php __(howMany("discuss","status","spam"))?>)</span></a>&nbsp;
		|&nbsp;<a href="./?zona=discutii&pagina=deleted" class="current">Mesaje sterse <span>(<?php __(howMany("discuss","status","deleted"))?>)</span></a>
	</div>
	...
	<table border="1" class="table_data" id="discuss">
		...
		<tbody>
		<?php
		$q=mysql_query("SELECT * FROM `discuss` WHERE `status`='deleted' ORDER by `data` DESC");
		$i=0;
		...
		?>
			<tr <?php __($trClass)?>>
				...
				<td style="width: 10%;text-align: center;clear: both;">
					...
					<input type="button" class="ui-admin ui-delete" title="Sterge" value="&nbsp;" onClick="msgConfirmFDelete('<?php __($r['id'])?>')"/>
				</td>
			</tr>
			<?php }?>
		</tbody>
		<tfoot>
			<tr>
				...
				<select name="action">
					...
					<option value="mfdelete">Sterge</option>
				</select>
