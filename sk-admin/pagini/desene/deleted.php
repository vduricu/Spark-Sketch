<div class="content adminContent">
	<div class="aSMenu">
		...
		|&nbsp;<a href="./?zona=desene&pagina=deleted" class="current">Desene Sterse <span>(<?php __(howMany("draws","status","deleted"))?>)</span></a>
	</div>
	...
	<table border="1" class="table_data" id="draws">
		...
		<tbody>
		<?php
		$q=mysql_query("SELECT * FROM `draws` WHERE `status`='deleted' ORDER by `modificationdate` DESC");
		$i=0;
		...
		?>
			<tr <?php __($trClass)?>>
				...
				<td style="width: 9%;text-align: center;clear: both;">
					...
					<input type="button" class="ui-admin ui-delete" title="Sterge" value="&nbsp;" onClick="confirmFDelete('<?php __($r['filename'])?>')"/>
				</td>
			</tr>
		<?php }?>
		</tbody>
